...
class App:
    ...
    # SGL config.
    device_type = DEFAULT_DEVICE_MAP.get(platform.system(), spy.DeviceType.vulkan)
    ...
    _imgui_ui_scale: BehaviorSubject[int] = BehaviorSubject(1)
    ...
    def __init__(self, user_shader_paths: List[Path] = []) -> None:
        ...
        self.adapter = ImguiAdapter(
            self.window, self.device, self._imgui_ui_scale.value
        )
        ...
        self._curr_window_size.on_next(
            self._get_display_size(self.window.width, self.window.height)
        )
        ...
        # Subscribe to ImGui UI scale changes.
        self._imgui_ui_scale.subscribe(self.on_imgui_ui_scale_changed)
    ...
    def _reload_font(self) -> None:
        ...
        self.io.fonts.add_font_from_file_ttf(
            str(self._font_path), self._font_size * font_scale
        )
        self.adapter.refresh_font_texture()
        self.io.font_global_scale = 1.0 / font_scale
    ...
    def on_imgui_ui_scale_changed(self, imgui_ui_scale: int) -> None:
        self.adapter.imgui_ui_scale = imgui_ui_scale
        self.adapter.resize(self.window.width, self.window.height)
        self._curr_window_size.on_next(
            self._get_display_size(self.window.width, self.window.height)
        )
        self._need_font_reload = True
    ...
    def on_resize(self, width: int, height: int) -> None:
        self.adapter.imgui_ui_scale = self._imgui_ui_scale.value
        self.adapter.resize(width, height)
        self._curr_window_size.on_next(self._get_display_size(width, height))

    def _get_display_size(self, width: int, height: int) -> glm.ivec2:
        imgui_ui_scale = max(float(self._imgui_ui_scale.value), 1.0)
        return glm.ivec2(int(width / imgui_ui_scale), int(height / imgui_ui_scale))

    def _get_font_scale(self) -> float:
        imgui_ui_scale = max(float(self._imgui_ui_scale.value), 1.0)
        framebuffer_scale = max(self.adapter.framebuffer_scale)
        return imgui_ui_scale * framebuffer_scale
    ...
    async def run(self) -> None:
        while not self.window.should_close():
            ...
            if self.adapter.update_framebuffer_scale():
                self._need_font_reload = True
            # Start ImGui frame.
            imgui.new_frame()
