...
vec3 Phong_BRDF(vec3 L, vec3 V, vec3 N, vec3 diffuse_color, vec3 specular_color, float specular_exponent)
{
    ...
    vec3 R = reflect(-L, N);
    float specular = pow(max(dot(R, V), 0.0), specular_exponent);
    return diffuse_color * NdotL + specular_color * specular;
}
...
void main(void)
{
    ...
    if (useNormalMapping)
    {
        ...
        mat3 TBN = mat3(T, B, N);
        vec3 tangentNormal = texture(normalTextureSampler, texcoord).rgb;
