...
Matrix4x4 createWorldToCameraMatrix(const Vector3D& eye, const Vector3D& at, const Vector3D& up) {
  ...
  // Compute the matrix that transforms a point in world space to a point in camera space.
    Vector3D dir = (eye - at).unit();       // camera +Z
    Vector3D right = cross(up, dir).unit(); // camera +X
    Vector3D camUp = cross(dir, right);     // camera +Y

    double mat[16] = {};

    // Logical ROWS of world-to-camera rotation
    mat[0] = right.x;
    mat[1] = right.y;
    mat[2] = right.z;
